...
export const usersRouter = router({
  ...
  getAll: adminProcedure
    ...
    .query(async ({ input }): Promise<UserListResponse> => {
        console.log("[tRPC usersRouter.getAll] Input:", input);
        try {
  ...
  update: adminProcedure
    .input(z.object({
      ...
      data: updateUserSchema, // Usa schema de update
    }))
    .mutation(async ({ input, ctx }) => {
       console.log(`[tRPC usersRouter.update] ID: ${input.id}, Data:`, JSON.stringify(input.data));
  ...
  delete: adminProcedure
    .input(z.object({ id: z.string().refine(id => mongoose.isValidObjectId(id), { message: 'ID inválido' }) }))
    .mutation(async ({ input, ctx }) => {
       console.log(`[tRPC delete] ID: ${input.id}, Admin: ${ctx.user.id}`);
